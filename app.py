...
st.markdown("엑셀 데이터를 한글 템플릿에 자동으로 채워넣어 개별 파일을 생성합니다.")

# --- 1. 템플릿 업로드 ---
st.header("1단계: 한글 템플릿 업로드")
st.markdown(
    "한글 파일에서 바꿀 부분을 `{{이름}}`, `{{직급}}` 처럼 이중 중괄호로 표시한 뒤 "
    "**HWPX 형식**으로 저장해주세요."
)
...
with col1:
    filename_pattern = st.text_input(
        "파일명 패턴",
        value="{이름}_output",
        help="엑셀 컬럼명을 중괄호로 감싸서 사용 (예: {이름}_{부서})",
    )
with col2:
    st.markdown("&nbsp;")  # spacer
    st.info("예시: `{이름}_output` → `홍길동_output.hwpx`")
...
if template_file and data_file:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with col_a:
            st.markdown("**템플릿 플레이스홀더:**")
            if placeholders:
                for p in placeholders:
                    st.markdown(f"- `{{{{{p}}}}}`")
            else:
                st.warning("플레이스홀더를 찾을 수 없습니다.")

        with col_b:
            st.markdown(f"**엑셀 데이터: {len(rows)}행**")
            excel_headers = list(rows[0].keys()) if rows else []
            matched = set(placeholders) & set(excel_headers)
            unmatched = set(placeholders) - set(excel_headers)
            st.markdown(f"매칭됨: {', '.join(matched) if matched else '없음'}")
            if unmatched:
                st.warning(f"매칭 안 됨: {', '.join(unmatched)}")
        ...
        if st.button("🚀 한글 파일 생성", type="primary", use_container_width=True):
            ...
            for i, row in enumerate(rows):
                ext = ".hwpx"
                filename = generate_output_filename(row, filename_pattern, ext, i)
                ...
                progress.progress((i + 1) / len(rows), text=f"처리 중... ({i + 1}/{len(rows)})")
